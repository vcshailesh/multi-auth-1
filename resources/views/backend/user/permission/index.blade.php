                            ...
                            <div class="card-header">
                                ...
                                <div class="card-tools"
                                    ...
                                    @if(auth()->user()->is_superadmin == 1 || auth()->user()->can('Create Permission'))
                                        <a href="{{ route('admin.user.permission.create') }}" title="Create Permission"
        ...
        $(function () {
            var oTable = $('#permission-table').DataTable({
                ...
                ajax: {
                    ...
                    url: '{{ route("admin.user.permission.lists") }}',
                    type: 'GET',
            ...
            $('#permission-table').on('click', '.status', function () {
                ...
                $.confirm({
                    ...
                    buttons: {
                        confirm: function () {
                            $.ajax({
                                url: '{{route('admin.user.permission.change-status')}}',
            ...
            $('#permission-table').on('click', '.deleteConfirmation', function () {
                ...
                $.confirm({
                    ...
                    buttons: {
                        confirm: function () {
                            $.ajax({
                                url: '{{route('admin.user.permission.destroy')}}',
                                data: {'id': ID, _token: '{{ csrf_token() }}'},
                                type: "DELETE",
                                success: function (data) {
                                    if (data.success == true) {
