                            ...
                            <div class="card-header">
                                ...
                                <div class="card-tools"
                                    ...
                                    <a href="javascript:void(0);"
                                       onclick="return deleteAction('role/bulk-action', 'role');"
                                       class="btn btn-primary" style="margin-right: 15px;">Submit</a>
                                    @if(auth()->user()->is_superadmin == 1 || auth()->user()->can('Create Role'))
                                        <a href="{{ route('admin.user.role.create') }}" title="Create Role"
        ...
        $(function () {
            var oTable = $('#role-table').DataTable({
                ...
                ajax: {
                    ...
                    url: '{{ route("admin.user.role.lists") }}',
                    type: 'GET',
            ...
            $('#role-table').on('click', "#parent", function () {
                $('.child').not(this).prop('checked', this.checked);
            });

            $('#role-table').on('click', '.child', function () {
            ...
            $('#role-table').on('click', '.status', function () {
                var ID = $(this).data('id');
                $.confirm({
                    title: 'Confirm!',
                    ...
                    buttons: {
                        confirm: function () {
                            $.ajax({
                                url: '{{route('admin.user.role.change-status')}}',
            ...
            $('#role-table').on('click', '.deleteConfirmation', function () {
                var ID = $(this).data('id');
                $.confirm({
                    title: 'Confirm!',
